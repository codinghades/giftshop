...
// https://github.com/ProgrammingInsider/Building-eCommerce-website-upload-Images-to-database-display-from-database-and-add-products-to-cart/blob/main/product%20table%20sql%20code.sql
    ...
    if(isset($_POST["submit"])){
        $arrivalname = $_POST["productname"]; // Changes 1
        $price = $_POST["price"];
        $discount = $_POST["discount"];


        // For Upload Photos
        $upload_dir = "newArrival/"; // this is where the uploaded photo stored // Changes 3
        $arrival_image = $upload_dir.$_FILES["imageUpload"]["name"];  // Changes 2
        ...
        if($upload_ok == 0){
            ...
            if($arrivalname != "" && $price !=""){ // changes 4
                move_uploaded_file($_FILES["imageUpload"]["tmp_name"], $upload_file);

                $sql = "INSERT INTO arrival(arrival_name,price,discount,arrival_image)
                VALUES('$arrivalname',$price,$discount,'$arrival_image')";
